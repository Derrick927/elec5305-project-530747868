...
def parse_args():
    ap = argparse.ArgumentParser(description="Plot DNN training curves from CSV log.")
...
def main():
    ...
    plt.plot(epochs, tr, label="Train BCE", 
    ...
    if has_val:
        plt.plot(epochs, [x if x==x else None for x in va], label="Val BCE",
                 linewidth=STYLE_CONFIG['linewidth'],
                 marker='s',
                 markersize=STYLE_CONFIG['markersize'],
    ...
    plt.ylabel("BCE Loss", fontsize=STYLE_CONFIG['fontsize']['label'], fontweight=STYLE_CONFIG['fontweight'])
    plt.title("DNN Training Curves", fontsize=STYLE_CONFIG['fontsize']['title'], fontweight=STYLE_CONFIG['fontweight'])
